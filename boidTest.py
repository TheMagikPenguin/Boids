...
class Boid(pygame.sprite.Sprite):
    ...
    def update(self, boids_group):
        ...
        if c_mass_loc is not None:
            ...
            if dx == 0: # don't divide by zero that'll halt motion, set to an really small number instead to give the
                        # illusion of no movement
                dx = 0.0000000000001
            if dy == 0:
                dy = 0.0000000000001
        ...
        ## Wrap screen
        # x
        if self.loc_next[0] >= 800:
            self.loc_next[0] = 800 - self.loc[0]
        elif self.loc_next[0] <= 0:
            self.loc_next[0] = self.loc[0] + 800
        #y
        if self.loc_next[1] >= 800:
            self.loc_next[1] = 800 - self.loc[1]
        elif self.loc_next[1] <= 0:
            self.loc_next[1] = self.loc[1] + 800
...
def main():
    ...
    for n in range(175):
        loc = [random.randint(200,600), random.randint(200,600)]
